$(function() {
	...
	var loadPointsExpenditureRecordListUri = "loadpointsexpenditurerecordlist";
	var paging = initPaging(".row-paging", loadPointsExpenditureRecordList);
	...
	function init() {
		//1、加载列表
		loadPointsExpenditureRecordList();
	...
	function loadPointsExpenditureRecordList() {
		$.showIndicator();
		$.ajax({
			url: loadPointsExpenditureRecordListUri,
			type: "GET",
			data: {
				shopId: shopId,
			...
			success: function(data, textStatus, jqXHR) {
				console.log("loadPointsExpenditureRecordListUri - returned data", data);
