...
CREATE TABLE IF NOT EXISTS `tb_area` (
  ...
  `priority` INT NOT NULL DEFAULT '0' COMMENT '权重（影响展示顺序）',
...
CREATE TABLE IF NOT EXISTS `tb_user_info` (
  ...
  `enable_status` TINYINT(2) NOT NULL DEFAULT '0' COMMENT '用户状态：0 禁止使用本商城，1 允许使用本商城',
  `user_type` TINYINT(2) NOT NULL DEFAULT '1' COMMENT '用户类型：1 顾客，2 店家，9 超级管理员 ',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `last_edit_time` DATETIME COMMENT '修改时间'
...
CREATE TABLE IF NOT EXISTS `tb_head_line` (
  ...
  `enable_status` TINYINT(2) NOT NULL DEFAULT '0' COMMENT '头条状态：0 不可用，1 可用',
  `priority` INT NOT NULL DEFAULT '0' COMMENT '权重（影响展示顺序）',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `last_edit_time` DATETIME COMMENT '修改时间'
...
CREATE TABLE IF NOT EXISTS `tb_shop` (
  ...
  `shop_category_id` INT COMMENT '店铺类别ID',
  ...
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `last_edit_time` DATETIME COMMENT '修改时间',
  `enable_status` TINYINT(2) NOT NULL DEFAULT '0' COMMENT '店铺状态：-1 不可用，0 审核中，1 可用',
...
CREATE TABLE IF NOT EXISTS `tb_product` (
  ...
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `last_edit_time` DATETIME COMMENT '修改时间',
  `enable_status` TINYINT(2) NOT NULL DEFAULT '0' COMMENT '商品状态：0 下架，1 在前端展示',
